...
public class MainActivity extends AppCompatActivity implements SensorEventListener {
    ...
    private void activityPrediction()
    {
        if(x.size() == Constants.N_SAMPLES && y.size() == Constants.N_SAMPLES && z.size() == Constants.N_SAMPLES) {
            ...
            if (results[activity_indx] > 0.7) {
                ...
                if (!current_activity.equals(activity_type) || activity_duration >= 1000) {
    ...
    public String[] predictedActivity(float[] results) {
        ...
        return new String[]{Integer.toString(predictedLabel), predictedString};
    }

    public void viewTable(View view){
        confidenceView = true;
        Log.d("Boolean Value ", Boolean.toString(true));
